...
var url;
	...
	function xiajiaShuju(){
		...
		var row=selectedRows[0];
		if(row.shujuType!=0){
			$.messager.alert("系统提示","不是空闲状态！");
			return;
		}
		url="../addShuju?shujuId="+row.shujuId;
		$.messager.confirm("系统提示","您确认要执行吗？",function(r){
			if(r){
				$.post(url,{shujuType:2},function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						$("#dg").datagrid("reload");
					}else{
...
<!--startprint-->
	...
	<div id="tb">
		<div>
			...
			<a href="javascript:shangjiaShuju()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">启用</a>
			<a href="javascript:xiajiaShuju()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">停用</a>
		</div>
		<div>
